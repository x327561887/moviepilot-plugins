...
class PersonMeta(_PluginBase):
    ...
    def __update_peoples(self, server: str, server_type: str,
        ...
        for people in iteminfo.get("People", []) or []:
            ...
            if not people.get("Name"):
                continue
            # 只处理演员，过滤掉导演/编剧等其他角色，原样保留
            if people.get("Type") and people.get("Type") != "Actor":
        ...
        def __need_trans_actor(_item):
            ...
            if self._type == "name":
                ...
                _peoples = [x for x in _item.get("People", []) if
                            x.get("Type") == "Actor"
                            and x.get("Name")
                            and not StringUtils.is_chinese(x.get("Name"))]
            elif self._type == "role":
                # 是否需要处理演员角色
                _peoples = [x for x in _item.get("People", []) if
                            x.get("Type") == "Actor"
                            and x.get("Role")
                            and not StringUtils.is_chinese(x.get("Role"))]
            else:
                _peoples = [x for x in _item.get("People", []) if
                            x.get("Type") == "Actor"
                            and ((x.get("Name") and not StringUtils.is_chinese(x.get("Name")))
        ...
        try:
            ...
            """
            {
              ...
              "avatar": {
                ...
                "normal": "https://qnmob3.doubanio.com/view/celebrity/raw/public/p42588.jpg?imageView2/2/q/80/w/80/format/webp"
              },
              "sharing_url": "https://www.douban.com/doubanapp/dispatch?uri=1025175/",
              "type": "celebrity",
              "id": "1025175",
    ...
    def __get_douban_actors(self, mediainfo: MediaInfo, season: int = None) -> List[dict]:
        ...
        doubaninfo = self.chain.match_doubaninfo(name=mediainfo.title,
                                                 ...
                                                 year=mediainfo.year,
                                                 season=season)
        # 豆瓣演员（只保留演员角色，排除导演/编剧等）
        if doubaninfo:
            doubanitem = self.chain.douban_info(doubaninfo.get("id")) or {}
            actors = doubanitem.get("actors") or []
            # 优先按 roles 字段过滤；空 roles 的情况下兜底保留全部 actors
            filtered = [a for a in actors
                        if a.get("roles") and "演员" in a.get("roles")]
            return filtered or actors
        else:
